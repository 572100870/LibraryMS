...
1. - 设计模型生成
     ...
     ```markdown
     根据以下需求生成面向对象设计模型：
     1. 用户可通过系统查询商品库存、下单、支付。
     2. 系统需管理商品信息、订单状态、支付流程。
     3. 核心实体包括商品、订单、用户。
     要求：生成类图（类名、属性、方法）、模块划分及交互逻辑。
     ```
   ...
   - 微服务拆分
     ...
     ```markdown
     基于以下需求，设计微服务架构：
     1. 系统需支持商品管理、订单处理、支付功能。
     2. 高并发场景下需保证库存一致性。
     要求：按业务功能拆分微服务，定义服务职责及通信方式（如REST API）。
     ```
...
2. **生成结果示例**：
   ...
   - 设计模型
     ...
     ```text
     类图：
     - 商品类（Product）：属性包括ID、名称、价格、库存；方法包括查询库存()、更新库存()。
     - 订单类（Order）：属性包括订单ID、商品列表、总金额；方法包括生成订单()、计算总额()。
     模块划分：
     - 商品管理模块：处理商品查询、库存更新。
     - 订单处理模块：处理订单创建、状态跟踪。
     ```
   ...
   - 微服务架构
     ...
     ```
     微服务拆分方案：
     - UserService：负责用户认证、权限管理（REST API）。
     - ProductService：管理商品信息、库存查询（gRPC）。
     - OrderService：处理订单创建、支付状态同步（消息队列触发库存扣减）。
     ```
...
1. 登录验证需实现3次失败锁定机制（功能需求）
2. 搜索服务需支持相关性排序算法（功能需求）
3. 书签保存需检测重复项（功能需求）
4. 系统需保证会话终止时的安全性（非功能需求：安全审计）
5. 邮件服务需具备重试机制（非功能需求：服务可靠性）
